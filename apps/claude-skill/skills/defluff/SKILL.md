---
name: defluff
description: Reverse the AI in corporate email. Detect whether the message was AI-written, AI-polished, or plainly human; when AI was involved, guess the prompt behind it. Classify urgency (ACTIONABLE / RESPONSE-NEEDED / FYI / NOISE) and extract 3–5 bullets of real intent. Detects invoice fraud / BEC, phishing, fake recruiters, fake interviews, conference scams, and crypto / MLM pitches with named red flags. Output mirrors the email's language. Handles single messages, threads, and batches.
when_to_use: Use when the user pastes or forwards one or more emails and asks for the point, a summary, action items, a triage pass, "who actually wrote this?", or "what do they actually want?". Also use for a forwarded thread, a dump of unread messages, or a suspicious-looking message the user wants screened.
---
